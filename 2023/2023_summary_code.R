...
output_statistics_basal <- input_data %>%
...
output_statistics_regeneration <- input_data %>%
  filter(!is.na(size_class)) %>%
  group_by(new_plot_key) %>%
  summarise(regeneration_present = ifelse(any(size_class == "sapling" | size_class == "seedling"), "Regeneration present", "Regeneration absent"))

# Step 6: Check for insect presence
output_statistics_insect <- input_data %>%
  group_by(new_plot_key) %>%
  summarise(insect_present = ifelse(any(insect_presence == 1), "Insect damage present", "Insect damage absent"))

# Step 7: Check for browse presence
output_statistics_browse <- input_data %>%
  group_by(new_plot_key) %>%
  summarise(browse_present = ifelse(any(browse_presence == 1), "Browse present", "Browse absent"))

# Step 8: Find the dominant tree species for each plot
output_statistics_species <- input_data %>%
